...
*/
set_current_record('LUSTRE_page', $this->LUSTRE_page);
?>

<p><a href="<?php echo html_escape(record_url($LUSTRE_page)); ?>">
<?php echo metadata('LUSTRE_page', 'title'); ?></a> 
 (<?php echo metadata('LUSTRE_page', 'slug'); ?>)<br/> 
 <?php echo __('<strong>%1$s</strong> on %2$s',
                html_escape(metadata('LUSTRE_page', 'modified_username')),
                html_escape(format_date(metadata('LUSTRE_page', 'updated'), Zend_Date::DATETIME_SHORT))); ?>
 <a class="edit" href="<?php echo html_escape(record_url($LUSTRE_page, 'edit')) ?>"><?php echo __('Edit'); ?></a><br/>
 <?php echo (metadata('LUSTRE_page', 'is_published') ? __('Published') : __('Not Published')); ?>
</p>
